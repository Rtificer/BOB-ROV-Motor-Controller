...
This should automatically flash the target pico with the motor-controller code. You should be able to read errors and warnings through the pico probe.\

To run tests simply run:\
`cargo test`

## Configuration
In [motor-controller/src/config.rs](motor-controller/src/config.rs) the configuration is split between i2c, dshot, and telemetry. Simply edit the values where the define_{telemetry_type}_config! macros are called. Example:
```rust
define_i2c_config! {
    peripheral: I2C0,
    scl_pin: PIN_1,
